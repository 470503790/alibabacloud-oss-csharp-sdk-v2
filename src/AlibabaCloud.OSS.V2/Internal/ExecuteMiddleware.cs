...
namespace AlibabaCloud.OSS.V2.Internal
{
    internal class TransportExecuteMiddleware : IExecuteMiddleware
    {
        ...
        public ResponseMessage Execute(RequestMessage request, ExecuteContext context)
        {
            ...
            try
            {
                httpResponse = handler.Send(httpRequest, completionOption, linkedCts.Token);
...
#if NET5_0_OR_GREATER
                    contentStream = httpResponse.Content.ReadAsStream();
#else
                    contentStream = httpResponse.Content.ReadAsStreamAsync().GetAwaiter().GetResult();
#endif
    ...
    internal class ResponseCheckerExecuteMiddleware : IExecuteMiddleware
    {
        ...
        public ResponseMessage Execute(RequestMessage request, ExecuteContext context)
        {
            var response = nextHandler.Execute(request, context);
            OnResponseMessage(response, context);
            return response;
        }
    ...
    internal class RetryerExecuteMiddleware : IExecuteMiddleware
    {
        ...
        public ResponseMessage Execute(RequestMessage request, ExecuteContext context)
        {
            ...
            for (var i = 0; ; i++)
            {
                try
                {
                    return _nextHandler.Execute(request, context);
                }
                catch (Exception ex)
                {
                    lastError = ex;
